// generated from rosidl_typesupport_connext_c/resource/idl__dds_connext__type_support_c.cpp.em
// with input from mymsgs:msg/Heart.idl
// generated code does not contain a copyright notice

#include <cassert>
#include <limits>

#include "mymsgs/msg/heart__rosidl_typesupport_connext_c.h"
#include "rcutils/types/uint8_array.h"
#include "rosidl_typesupport_connext_c/identifier.h"
#include "rosidl_typesupport_connext_c/wstring_conversion.hpp"
#include "rosidl_typesupport_connext_cpp/message_type_support.h"
#include "mymsgs/msg/rosidl_typesupport_connext_c__visibility_control.h"
#include "mymsgs/msg/detail/heart__struct.h"
#include "mymsgs/msg/detail/heart__functions.h"

#ifndef _WIN32
# pragma GCC diagnostic push
# pragma GCC diagnostic ignored "-Wunused-parameter"
# ifdef __clang__
#  pragma clang diagnostic ignored "-Wdeprecated-register"
#  pragma clang diagnostic ignored "-Wreturn-type-c-linkage"
# endif
#endif

#include "mymsgs/msg/dds_connext/Heart_Support.h"
#include "mymsgs/msg/dds_connext/Heart_Plugin.h"

#ifndef _WIN32
# pragma GCC diagnostic pop
#endif

// includes and forward declarations of message dependencies and their conversion functions
#if defined(__cplusplus)
extern "C"
{
#endif


// forward declare type support functions

static DDS_TypeCode *
_Heart__get_type_code()
{
  return mymsgs::msg::dds_::Heart_TypeSupport::get_typecode();
}

static bool
_Heart__convert_ros_to_dds(const void * untyped_ros_message, void * untyped_dds_message)
{
  if (!untyped_ros_message) {
    fprintf(stderr, "ros message handle is null\n");
    return false;
  }
  if (!untyped_dds_message) {
    fprintf(stderr, "dds message handle is null\n");
    return false;
  }
  const mymsgs__msg__Heart * ros_message =
    static_cast<const mymsgs__msg__Heart *>(untyped_ros_message);
  mymsgs::msg::dds_::Heart_ * dds_message =
    static_cast<mymsgs::msg::dds_::Heart_ *>(untyped_dds_message);
  // Member name: error_code
  {
    dds_message->error_code_ = ros_message->error_code;
  }
  // Member name: type
  {
    dds_message->type_ = ros_message->type;
  }
  // Member name: base_mode
  {
    dds_message->base_mode_ = ros_message->base_mode;
  }
  // Member name: system_status
  {
    dds_message->system_status_ = ros_message->system_status;
  }
  return true;
}

static bool
_Heart__convert_dds_to_ros(const void * untyped_dds_message, void * untyped_ros_message)
{
  if (!untyped_ros_message) {
    fprintf(stderr, "ros message handle is null\n");
    return false;
  }
  if (!untyped_dds_message) {
    fprintf(stderr, "dds message handle is null\n");
    return false;
  }
  const mymsgs::msg::dds_::Heart_ * dds_message =
    static_cast<const mymsgs::msg::dds_::Heart_ *>(untyped_dds_message);
  mymsgs__msg__Heart * ros_message =
    static_cast<mymsgs__msg__Heart *>(untyped_ros_message);
  // Member name: error_code
  {
    ros_message->error_code = dds_message->error_code_;
  }
  // Member name: type
  {
    ros_message->type = dds_message->type_;
  }
  // Member name: base_mode
  {
    ros_message->base_mode = dds_message->base_mode_;
  }
  // Member name: system_status
  {
    ros_message->system_status = dds_message->system_status_;
  }
  return true;
}


static bool
_Heart__to_cdr_stream(
  const void * untyped_ros_message,
  rcutils_uint8_array_t * cdr_stream)
{
  if (!untyped_ros_message) {
    return false;
  }
  if (!cdr_stream) {
    return false;
  }
  const mymsgs__msg__Heart * ros_message =
    static_cast<const mymsgs__msg__Heart *>(untyped_ros_message);
  mymsgs::msg::dds_::Heart_ dds_message;
  if (!_Heart__convert_ros_to_dds(ros_message, &dds_message)) {
    return false;
  }

  // call the serialize function for the first time to get the expected length of the message
  unsigned int expected_length;
  if (mymsgs::msg::dds_::Heart_Plugin_serialize_to_cdr_buffer(
      NULL, &expected_length, &dds_message) != RTI_TRUE)
  {
    fprintf(stderr, "failed to call mymsgs::msg::dds_::Heart_Plugin_serialize_to_cdr_buffer()\n");
    return false;
  }
  if (expected_length > (std::numeric_limits<unsigned int>::max)()) {
    fprintf(stderr, "expected_length, unexpectedly larger than max unsigned int\n");
    return false;
  }
  if (cdr_stream->buffer_capacity < expected_length) {
    uint8_t * new_buffer = static_cast<uint8_t *>(cdr_stream->allocator.allocate(expected_length, cdr_stream->allocator.state));
    if (NULL == new_buffer) {
      fprintf(stderr, "failed to allocate memory for cdr data\n");
      return false;
    }
    cdr_stream->allocator.deallocate(cdr_stream->buffer, cdr_stream->allocator.state);
    cdr_stream->buffer = new_buffer;
    cdr_stream->buffer_capacity = expected_length;
  }
  // call the function again and fill the buffer this time
  unsigned int buffer_length_uint = static_cast<unsigned int>(expected_length);
  if (mymsgs::msg::dds_::Heart_Plugin_serialize_to_cdr_buffer(
      reinterpret_cast<char *>(cdr_stream->buffer),
      &buffer_length_uint,
      &dds_message) != RTI_TRUE)
  {
    cdr_stream->buffer_length = 0;
    return false;
  }
  cdr_stream->buffer_length = expected_length;

  return true;
}

static bool
_Heart__to_message(
  const rcutils_uint8_array_t * cdr_stream,
  void * untyped_ros_message)
{
  if (!cdr_stream) {
    return false;
  }
  if (!untyped_ros_message) {
    return false;
  }

  mymsgs::msg::dds_::Heart_ * dds_message =
    mymsgs::msg::dds_::Heart_TypeSupport::create_data();
  if (cdr_stream->buffer_length > (std::numeric_limits<unsigned int>::max)()) {
    fprintf(stderr, "cdr_stream->buffer_length, unexpectedly larger than max unsigned int\n");
    return false;
  }
  if (mymsgs::msg::dds_::Heart_Plugin_deserialize_from_cdr_buffer(
      dds_message,
      reinterpret_cast<char *>(cdr_stream->buffer),
      static_cast<unsigned int>(cdr_stream->buffer_length)) != RTI_TRUE)
  {
    fprintf(stderr, "deserialize from cdr buffer failed\n");
    return false;
  }
  bool success = _Heart__convert_dds_to_ros(dds_message, untyped_ros_message);
  if (mymsgs::msg::dds_::Heart_TypeSupport::delete_data(dds_message) != DDS_RETCODE_OK) {
    return false;
  }
  return success;
}

static message_type_support_callbacks_t _Heart__callbacks = {
  "mymsgs::msg",  // message_namespace
  "Heart",  // message_name
  _Heart__get_type_code,  // get_type_code
  _Heart__convert_ros_to_dds,  // convert_ros_to_dds
  _Heart__convert_dds_to_ros,  // convert_dds_to_ros
  _Heart__to_cdr_stream,  // to_cdr_stream
  _Heart__to_message  // to_message
};

static rosidl_message_type_support_t _Heart__type_support = {
  rosidl_typesupport_connext_c__identifier,
  &_Heart__callbacks,
  get_message_typesupport_handle_function,
};

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(
  rosidl_typesupport_connext_c,
  mymsgs, msg,
  Heart)()
{
  return &_Heart__type_support;
}

#if defined(__cplusplus)
}
#endif
