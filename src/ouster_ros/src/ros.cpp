...
using namespace std;
bool read_imu_packet(const sensor::client& cli, mymsgs::msg::PacketMsg & m,
                     const sensor::packet_format& pf) {
    m.buf.resize(pf.imu_packet_size + 1);
    return read_imu_packet(cli, m.buf.data(), pf);
}

bool read_lidar_packet(const sensor::client& cli, mymsgs::msg::PacketMsg & m,
                       const sensor::packet_format& pf) {
    m.buf.resize(pf.lidar_packet_size + 1);
    return read_lidar_packet(cli, m.buf.data(), pf);
}

sensor_msgs::msg::Imu packet_to_imu_msg(const mymsgs::msg::PacketMsg & p, const std::string& frame,
                                   const sensor::packet_format& pf) {
...
sensor_msgs::msg::PointCloud2 cloud_to_cloud_msg(const Cloud& cloud, ns timestamp,
